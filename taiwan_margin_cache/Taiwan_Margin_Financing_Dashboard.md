...
- threshold_policy: percentile｜calib_min_n=60
  ...
  - calibration_status:
    - expansion20: status=FALLBACK_FIXED, sample_n=33, threshold=8.0, reason=insufficient samples (n=33 < calib_min_n=60)
    - contraction20: status=FALLBACK_FIXED, sample_n=33, threshold=-8.0, reason=insufficient samples (n=33 < calib_min_n=60)
    - watch1d: status=FALLBACK_FIXED, sample_n=52, threshold=0.8, reason=insufficient samples (n=52 < calib_min_n=60)
    - watchspread20: status=FALLBACK_FIXED, sample_n=33, threshold=3.0, reason=insufficient samples (n=33 < calib_min_n=60)
    - watchaccel: status=FALLBACK_FIXED, sample_n=48, threshold=0.25, reason=insufficient samples (n=48 < calib_min_n=60)
...
## 2) 資料
- 上市(TWSE)：融資餘額 3826.80 億元｜資料日期 2026-03-06｜來源：HiStock（https://histock.tw/stock/three.aspx?m=mg）
  - rows_latest_table=30｜rows_series=53｜head_dates=['2026-03-06', '2026-03-05', '2026-03-04']｜tail_dates=['2026-01-16', '2026-01-15', '2026-01-14']
- 上櫃(TPEX)：融資餘額 1393.10 億元｜資料日期 2026-03-06｜來源：HiStock（https://histock.tw/stock/three.aspx?m=mg&no=TWOI）
  - rows_latest_table=30｜rows_series=53｜head_dates=['2026-03-06', '2026-03-05', '2026-03-04']｜tail_dates=['2026-01-16', '2026-01-15', '2026-01-14']
- 合計：融資餘額 5219.90 億元｜資料日期 2026-03-06｜來源：TWSE=HiStock / TPEX=HiStock
...
- data_date: 2026-03-06｜maint_ratio_pct: 177.554105
- maint_ratio_1d_delta_pctpt: -0.851128｜maint_ratio_1d_pct_change: -0.477076
- maint_ratio_trend_note: trend_from: today=177.554105(2026-03-06), prev=178.405233(2026-03-05)

## 2.1) 台股成交量/波動（roll25_cache；confirm-only）
- roll25_path: roll25_cache/latest_report.json
- UsedDate: 2026-03-05｜UsedDateStatus: DATA_NOT_UPDATED｜risk_level: 低(derived)（stale）｜risk_level_raw: NA｜tag: WEEKDAY
...
### 上市(TWSE)
- 1D：Δ=35.10 億元；Δ%=0.9257 %｜latest=3826.80｜base=3791.70（基期日=2026-03-05）
### 上櫃(TPEX)
- 1D：Δ=9.20 億元；Δ%=0.6648 %｜latest=1393.10｜base=1383.90（基期日=2026-03-05）

## 3.1) OTC Guardrail（display-only；不影響主信號）
- stage: NONE｜label: NONE
- rationale: no OTC guardrail triggered
- inputs: TPEX_20D%=4.3912｜TPEX_1D%=0.6648｜TPEX_5D%=1.0591
...
- Check-4 TWSE history rows>=21：✅（PASS）（rows_series=53）
- Check-4 TPEX history rows>=21：✅（PASS）（rows_series=53）
- Check-5 TWSE 20D base_date 存在於 series：✅（PASS）
- Check-5 TPEX 20D base_date 存在於 series：✅（PASS）
- Check-6 roll25 UsedDate 與 TWSE 最新日期一致（confirm-only）：⚠️（NOTE）（roll25 stale (UsedDateStatus=DATA_NOT_UPDATED) | UsedDate(2026-03-05) vs TWSE(2026-03-06)）
...
_generated_at_utc: 2026-03-06T15:04:45Z_
