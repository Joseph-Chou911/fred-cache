...
- 狀態：擴張｜信號：WATCH｜資料品質：OK
  - rationale: 20D expansion + 1D%>=0.8 (leveraging acceleration)
...
- Check-4 TWSE history rows>=21：✅（OK）（rows=30）
- Check-4 TPEX history rows>=21：✅（OK）（rows=30）
- Check-5 TWSE 20D base_date 存在於 series：✅（OK）（OK）
- Check-5 TPEX 20D base_date 存在於 series：✅（OK）（OK）

_generated_at_utc: 2026-01-23T06:53:25Z_
