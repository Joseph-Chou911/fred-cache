...
## 15秒摘要
- **VT** (2026-02-20 price_usd=147.9500) → **NEAR_UPPER_BAND** (z=1.5954, pos=0.8989, pos_raw=0.8989); dist_to_lower=7.172%; dist_to_upper=0.841%; 20D forward_mdd: p50=-1.577%, p10=-5.620%, p5=-6.902%, min=-27.390% (n=828, conf=HIGH, conf_decision=OK, min_n_required=200)
...
- Δpos_1d: 0.0981
- Δpos_raw_1d: 0.0981
- Δband_width_1d: -0.368%
- Δdist_to_upper_1d: -0.891%

## 解讀重點（更詳盡）
- **Band 位置**：pos=0.8989（clipped） / pos_raw=0.8989（未截斷；突破上下軌時用於稽核）
- **距離上下軌**：dist_to_upper=0.841%；dist_to_lower=7.172%
- **波動區間寬度（閱讀用）**：band_width≈8.632%（= upper/lower - 1；用於直覺理解，不作信號）
...
- expected_dist_to_upper(logband): `0.841%`
...
- band_width_current: 8.632%; percentile≈36.42; current_bin=`B2(p20-40]`
...
| field | value | note |
|---|---:|---|
| price_usd | 147.9500 | adj_close |
| close_usd | 147.9500 | raw close (for reference) |
| z | 1.5954 | log(price) z-score vs BB mean/stdev |
| pos_in_band | 0.8989 | clipped [0,1] for readability |
| pos_raw | 0.8989 | NOT clipped; can be <0 or >1 when price breaks bands |
| lower_usd | 137.3387 | exp(lower_log) |
| upper_usd | 149.1942 | exp(upper_log) |
| dist_to_lower | 7.172% | (price-lower)/price |
| dist_to_upper | 0.841% | (upper-price)/price |
| band_width | 8.632% | (upper/lower - 1) reading-only |
