...
- threshold_policy: percentile｜calib_min_n=60
  ...
  - calibration_status:
    - expansion20: status=FALLBACK_FIXED, sample_n=29, threshold=8.0, reason=insufficient samples (n=29 < calib_min_n=60)
    - contraction20: status=FALLBACK_FIXED, sample_n=29, threshold=-8.0, reason=insufficient samples (n=29 < calib_min_n=60)
    - watch1d: status=FALLBACK_FIXED, sample_n=48, threshold=0.8, reason=insufficient samples (n=48 < calib_min_n=60)
    - watchspread20: status=FALLBACK_FIXED, sample_n=29, threshold=3.0, reason=insufficient samples (n=29 < calib_min_n=60)
    - watchaccel: status=FALLBACK_FIXED, sample_n=44, threshold=0.25, reason=insufficient samples (n=44 < calib_min_n=60)
...
## 2) 資料
- 上市(TWSE)：融資餘額 3916.20 億元｜資料日期 2026-03-02｜來源：HiStock（https://histock.tw/stock/three.aspx?m=mg）
  - rows_latest_table=30｜rows_series=49｜head_dates=['2026-03-02', '2026-02-26', '2026-02-25']｜tail_dates=['2026-01-12', '2026-01-09', '2026-01-08']
- 上櫃(TPEX)：融資餘額 1389.10 億元｜資料日期 2026-03-02｜來源：HiStock（https://histock.tw/stock/three.aspx?m=mg&no=TWOI）
  - rows_latest_table=30｜rows_series=49｜head_dates=['2026-03-02', '2026-02-26', '2026-02-25']｜tail_dates=['2026-01-12', '2026-01-09', '2026-01-08']
- 合計：融資餘額 5305.30 億元｜資料日期 2026-03-02｜來源：TWSE=HiStock / TPEX=HiStock
...
- data_date: 2026-03-02｜maint_ratio_pct: 189.609343
- maint_ratio_1d_delta_pctpt: -0.659599｜maint_ratio_1d_pct_change: -0.346667
- maint_ratio_trend_note: trend_from: today=189.609343(2026-03-02), prev=190.268942(2026-02-26)

## 2.1) 台股成交量/波動（roll25_cache；confirm-only）
- roll25_path: roll25_cache/latest_report.json
- UsedDate: 2026-02-26｜UsedDateStatus: DATA_NOT_UPDATED｜risk_level: 低(derived)（stale）｜risk_level_raw: NA｜tag: WEEKDAY
...
### 上市(TWSE)
- 1D：Δ=17.60 億元；Δ%=0.4514 %｜latest=3916.20｜base=3898.60（基期日=2026-02-26）
### 上櫃(TPEX)
- 1D：Δ=10.60 億元；Δ%=0.7690 %｜latest=1389.10｜base=1378.50（基期日=2026-02-26）

## 3.1) OTC Guardrail（display-only；不影響主信號）
- stage: NONE｜label: NONE
- rationale: no OTC guardrail triggered
- inputs: TPEX_20D%=7.5321｜TPEX_1D%=0.7690｜TPEX_5D%=5.7717
...
- Check-4 TWSE history rows>=21：✅（PASS）（rows_series=49）
- Check-4 TPEX history rows>=21：✅（PASS）（rows_series=49）
- Check-5 TWSE 20D base_date 存在於 series：✅（PASS）
- Check-5 TPEX 20D base_date 存在於 series：✅（PASS）
- Check-6 roll25 UsedDate 與 TWSE 最新日期一致（confirm-only）：⚠️（NOTE）（roll25 stale (UsedDateStatus=DATA_NOT_UPDATED) | UsedDate(2026-02-26) vs TWSE(2026-03-02)）
...
_generated_at_utc: 2026-03-02T15:07:49Z_
