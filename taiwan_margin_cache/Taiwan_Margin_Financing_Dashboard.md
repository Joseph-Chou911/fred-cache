...
- threshold_policy: percentile｜calib_min_n=60
  ...
  - calibration_status:
    - expansion20: status=FALLBACK_FIXED, sample_n=32, threshold=8.0, reason=insufficient samples (n=32 < calib_min_n=60)
    - contraction20: status=FALLBACK_FIXED, sample_n=32, threshold=-8.0, reason=insufficient samples (n=32 < calib_min_n=60)
    - watch1d: status=FALLBACK_FIXED, sample_n=51, threshold=0.8, reason=insufficient samples (n=51 < calib_min_n=60)
    - watchspread20: status=FALLBACK_FIXED, sample_n=32, threshold=3.0, reason=insufficient samples (n=32 < calib_min_n=60)
    - watchaccel: status=FALLBACK_FIXED, sample_n=47, threshold=0.25, reason=insufficient samples (n=47 < calib_min_n=60)
...
## 2) 資料
- 上市(TWSE)：融資餘額 3791.70 億元｜資料日期 2026-03-05｜來源：HiStock（https://histock.tw/stock/three.aspx?m=mg）
  - rows_latest_table=30｜rows_series=52｜head_dates=['2026-03-05', '2026-03-04', '2026-03-03']｜tail_dates=['2026-01-15', '2026-01-14', '2026-01-13']
- 上櫃(TPEX)：融資餘額 1383.90 億元｜資料日期 2026-03-05｜來源：HiStock（https://histock.tw/stock/three.aspx?m=mg&no=TWOI）
  - rows_latest_table=30｜rows_series=52｜head_dates=['2026-03-05', '2026-03-04', '2026-03-03']｜tail_dates=['2026-01-15', '2026-01-14', '2026-01-13']
- 合計：融資餘額 5175.60 億元｜資料日期 2026-03-05｜來源：TWSE=HiStock / TPEX=HiStock
...
- data_date: 2026-03-05｜maint_ratio_pct: 178.405233
- maint_ratio_1d_delta_pctpt: 6.106070｜maint_ratio_1d_pct_change: 3.543877
- maint_ratio_trend_note: trend_from: today=178.405233(2026-03-05), prev=172.299163(2026-03-04)
...
### 上市(TWSE)
- 1D：Δ=25.50 億元；Δ%=0.6771 %｜latest=3791.70｜base=3766.20（基期日=2026-03-04）
### 上櫃(TPEX)
- 1D：Δ=18.90 億元；Δ%=1.3846 %｜latest=1383.90｜base=1365.00（基期日=2026-03-04）

## 3.1) OTC Guardrail（display-only；不影響主信號）
- stage: NONE｜label: NONE
- rationale: no OTC guardrail triggered
- inputs: TPEX_20D%=4.6111｜TPEX_1D%=1.3846｜TPEX_5D%=2.6251
...
- Check-4 TWSE history rows>=21：✅（PASS）（rows_series=52）
- Check-4 TPEX history rows>=21：✅（PASS）（rows_series=52）
- Check-5 TWSE 20D base_date 存在於 series：✅（PASS）
- Check-5 TPEX 20D base_date 存在於 series：✅（PASS）
- Check-6 roll25 UsedDate 與 TWSE 最新日期一致（confirm-only）：⚠️（NOTE）（roll25 stale (UsedDateStatus=DATA_NOT_UPDATED) | UsedDate(2026-03-05) == TWSE(2026-03-05)）
...
_generated_at_utc: 2026-03-05T23:11:05Z_
