...
def _safe_source_url(series_id: str) -> str:
    ...
    This URL is safe to store in CSV and logs.
    """
    params = {
        "series_id": series_id,
        "file_type": "json",
...
def _redact_secrets(s: str) -> str:
    ...
    - Redacts literal key if it somehow appears
    - Redacts any api_key=xxxxx pattern
    """
    if not s:
        return s
    ...
    # redact api_key query value (robust)
...
def fetch_latest_obs(series_id: str) -> dict:
    ...
    Never store/log the request URL (it can include api_key).
    """
    params = {
        "series_id": series_id,
...
def main() -> None:
    ...
    for sid in SERIES:
        try:
            ...
            # Don't include exception message; it may contain URLs.
            row = {
                "series_id": sid,
                "data_date": "NA",
                "value": "NA",
