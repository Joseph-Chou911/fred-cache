...
- report_generated_at_utc: `2026-03-02T23:43:47Z`
- data_date: `2026-03-02`
- price_mode: `adj_close`
- params: `BB(60,2.0) on log(price)`, `forward_mdd(20D)`, sidecar=`forward_mdd(10D)`

## 15秒摘要
- **VT** (2026-03-02 price_usd=146.9500) → **MID_BAND** (z=1.0152, pos=0.7538, pos_raw=0.7538); dist_to_lower=5.741%; dist_to_upper=1.950%; 20D forward_mdd: p50=-1.636%, p10=-7.278%, p5=-10.128%, min=-31.571% (n=2897, conf=HIGH, conf_decision=OK, min_n_required=200)

## Δ1D（一日變動；以前一個「可計算 BB 的交易日」為基準）
- prev_bb_date: `2026-02-27`
- Δprice_1d: -0.602%
- Δz_1d: -0.3341
- Δpos_1d: -0.0835
- Δpos_raw_1d: -0.0835
- Δband_width_1d: -0.119%
- Δdist_to_upper_1d: 0.647%

## 解讀重點（更詳盡）
- **Band 位置**：pos=0.7538（clipped） / pos_raw=0.7538（未截斷；突破上下軌時用於稽核）
- **距離上下軌**：dist_to_upper=1.950%；dist_to_lower=5.741%
- **波動區間寬度（閱讀用）**：band_width≈8.159%（= upper/lower - 1；用於直覺理解，不作信號）
- **streak（連續天數）**：bucket_streak=2；pos≥0.80 streak=0；dist_to_upper≤2.0% streak=9
...
- expected_dist_to_upper(logband): `1.950%`
- abs_err: `0.00000000`; abs_tol: `0.00010000`
- rel_err: `0.000000`; rel_tolerance: `0.020000`

## forward_mdd(20D) 切片分布（閱讀用；不回填主欄位）

- Slice A（pos≥0.80）：p50=-1.509%、p10=-5.424%、p5=-6.990%、min=-31.285% (n=1711, conf=HIGH, conf_decision=OK, min_n_required=200)
- Slice B（dist_to_upper≤2.0%）：p50=-1.445%、p10=-5.481%、p5=-6.991%、min=-31.285% (n=1750, conf=HIGH, conf_decision=OK, min_n_required=200)
...
- bucket=MID_BAND：p50=-0.990%、p10=-4.894%、p5=-6.660%、min=-27.590% (n=2903, conf=HIGH, conf_decision=OK)
...
- band_width_current: 8.159%; percentile≈31.93; current_bin=`B2(p20-40]`
- quantiles: p20=6.873%, p40=8.963%, p50=9.847%, p60=11.425%, p80=16.695% (n_bw_samples=4388)
- current_bin streak=7
...
| B3(p40-60] | 865 | -1.817% | -6.431% | -8.638% | -28.032% | HIGH | OK |
...
| B3(p40-60] | 875 | -0.956% | -4.315% | -5.373% | -24.197% | HIGH | OK |
...
| bw_bin | n | p50 | p10 | p5 | min | conf | conf_decision |
|---|---:|---:|---:|---:|---:|---|---|
| B1(<=p20) | 571 | -0.785% | -4.022% | -5.515% | -10.991% | HIGH | OK |
| B2(p20-40] | 573 | -0.735% | -4.099% | -5.530% | -17.100% | HIGH | OK |
| B3(p40-60] | 571 | -1.088% | -4.620% | -5.403% | -7.447% | HIGH | OK |
...
| field | value | note |
|---|---:|---|
| price_usd | 146.9500 | adj_close |
| close_usd | 146.9500 | raw close (for reference) |
| z | 1.0152 | log(price) z-score vs BB mean/stdev |
| pos_in_band | 0.7538 | clipped [0,1] for readability |
| pos_raw | 0.7538 | NOT clipped; can be <0 or >1 when price breaks bands |
| lower_usd | 138.5141 | exp(lower_log) |
| upper_usd | 149.8150 | exp(upper_log) |
| dist_to_lower | 5.741% | (price-lower)/price |
| dist_to_upper | 1.950% | (upper-price)/price |
| band_width | 8.159% | (upper/lower - 1) reading-only |
...
- fx_rate_strict (for 2026-03-02): `31.4500`
- derived price_twd (strict): `4621.58`
