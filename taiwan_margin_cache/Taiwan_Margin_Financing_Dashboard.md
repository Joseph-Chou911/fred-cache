...
- threshold_policy: percentile｜calib_min_n=60
  ...
  - calibration_status:
    - expansion20: status=FALLBACK_FIXED, sample_n=27, threshold=8.0, reason=insufficient samples (n=27 < calib_min_n=60)
    - contraction20: status=FALLBACK_FIXED, sample_n=27, threshold=-8.0, reason=insufficient samples (n=27 < calib_min_n=60)
    - watch1d: status=FALLBACK_FIXED, sample_n=46, threshold=0.8, reason=insufficient samples (n=46 < calib_min_n=60)
    - watchspread20: status=FALLBACK_FIXED, sample_n=27, threshold=3.0, reason=insufficient samples (n=27 < calib_min_n=60)
    - watchaccel: status=FALLBACK_FIXED, sample_n=42, threshold=0.25, reason=insufficient samples (n=42 < calib_min_n=60)
...
## 2) 資料
- 上市(TWSE)：融資餘額 3831.30 億元｜資料日期 2026-02-25｜來源：HiStock（https://histock.tw/stock/three.aspx?m=mg）
  - rows_latest_table=30｜rows_series=47｜head_dates=['2026-02-25', '2026-02-24', '2026-02-23']｜tail_dates=['2026-01-08', '2026-01-07', '2026-01-06']
- 上櫃(TPEX)：融資餘額 1348.50 億元｜資料日期 2026-02-25｜來源：HiStock（https://histock.tw/stock/three.aspx?m=mg&no=TWOI）
  - rows_latest_table=30｜rows_series=47｜head_dates=['2026-02-25', '2026-02-24', '2026-02-23']｜tail_dates=['2026-01-08', '2026-01-07', '2026-01-06']
- 合計：融資餘額 5179.80 億元｜資料日期 2026-02-25｜來源：TWSE=HiStock / TPEX=HiStock
...
- data_date: 2026-02-25｜maint_ratio_pct: 189.37103
- maint_ratio_1d_delta_pctpt: 0.676285｜maint_ratio_1d_pct_change: 0.358402
- maint_ratio_trend_note: trend_from: today=189.37103(2026-02-25), prev=188.694745(2026-02-24)

## 2.1) 台股成交量/波動（roll25_cache；confirm-only）
- roll25_path: roll25_cache/latest_report.json
- UsedDate: 2026-02-24｜UsedDateStatus: DATA_NOT_UPDATED｜risk_level: 低(derived)（stale）｜risk_level_raw: NA｜tag: WEEKDAY
...
### 上市(TWSE)
- 1D：Δ=56.30 億元；Δ%=1.4914 %｜latest=3831.30｜base=3775.00（基期日=2026-02-24）
### 上櫃(TPEX)
- 1D：Δ=12.10 億元；Δ%=0.9054 %｜latest=1348.50｜base=1336.40（基期日=2026-02-24）

## 3.1) OTC Guardrail（display-only；不影響主信號）
- stage: NONE｜label: NONE
- rationale: no OTC guardrail triggered
- inputs: TPEX_20D%=4.5754｜TPEX_1D%=0.9054｜TPEX_5D%=1.4749
...
- Check-4 TWSE history rows>=21：✅（PASS）（rows_series=47）
- Check-4 TPEX history rows>=21：✅（PASS）（rows_series=47）
- Check-5 TWSE 20D base_date 存在於 series：✅（PASS）
- Check-5 TPEX 20D base_date 存在於 series：✅（PASS）
- Check-6 roll25 UsedDate 與 TWSE 最新日期一致（confirm-only）：⚠️（NOTE）（roll25 stale (UsedDateStatus=DATA_NOT_UPDATED) | UsedDate(2026-02-24) vs TWSE(2026-02-25)）
...
_generated_at_utc: 2026-02-25T15:33:02Z_
